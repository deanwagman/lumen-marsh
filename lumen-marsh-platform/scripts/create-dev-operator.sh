#!/usr/bin/env bash
set -euo pipefail
# Create the development operator in the lumen-marsh-dev user pool.
...
EMAIL="${DEV_OPERATOR_EMAIL:-operator@lumen-marsh.dev}"
...
if aws cognito-idp admin-get-user --user-pool-id "$POOL_ID" --username "$EMAIL" >/dev/null 2>&1; then
  echo "operator already exists: $EMAIL"
else
...
merge_env_file "$ENV_FILE" \
  ...
  "DEV_OPERATOR_EMAIL=$EMAIL" \
  "DEV_OPERATOR_PASSWORD=$PASSWORD"

echo "operator is in group operators"
