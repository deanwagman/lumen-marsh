...
Starts the integrated Lumen Marsh stack with Docker Compose.
  ...
  --oidc       Use the applied development Cognito user pool. Starts VenueOps,
               Environmental Monitor, the operator console, and guest app.
...
if [[ "$AUTH_MODE" == "oidc" ]]; then
  ...
  if ! tofu -chdir="$identity_dir" output -raw CONSOLE_CLIENT_ID >/dev/null 2>&1 || \
     ! tofu -chdir="$identity_dir" output -raw MONITOR_SECRET_ARN >/dev/null 2>&1; then
  ...
  if [[ -z "$issuer" || "$issuer" == "null" || -z "$client" || "$client" == "null" || -z "$monitor_secret_arn" || "$monitor_secret_arn" == "null" ]]; then
  ...
  export VENUEOPS_ALLOWED_CLIENT_IDS="$client,$monitor_client"
  ...
  SERVICES=(venueops-api environmental-monitor venueops-console lumen-marsh-app)
fi
